  ...
  contextflow: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Before anything is written, ContextFlow classifies who the recipient actually is — executive, founder, hiring manager, peer, or recruiter — because that classification changes what follows, not just the tone of a template. Each audience type has its own generation logic: what to emphasize, what from your background is actually relevant to that kind of person, and how to open the message. A separate classification checks how close the recipient's own role is to the one you're pursuing — same field, an adjacent one, or a genuinely different one — so a message to someone outside your field doesn't assume technical overlap that isn't there. Context for generation is assembled from three places at once — who you are (profile, background, communication preferences), what the campaign is about (company, role, job description), and who you're writing to (the audience type and domain classification just resolved) — and once a draft exists, it's checked asynchronously in the background against that same context, flagging any claim the context doesn't actually support rather than gating whether the draft can be sent.",
      ...
      },
      {
        ...
        decisions: [
          ...
          {
            title: "Grounding/verification as a check on generated drafts",
            body: "A generated message can read as confident and specific while still being loosely connected to what the job posting or the recipient's actual background support. Rather than trusting a generated draft at face value, ContextFlow runs a background grounding-verification pass after a draft is generated and saved — checking each claim in it against the context it was supposed to be based on, and flagging anything unsupported. Today that's an observability signal, not a send-blocking gate. The point isn't that this makes every message perfect — it's that “the model produced text” and “the text is actually grounded in the right context” are treated as two different things, checked mechanically rather than assumed to be the same.",
          },
          {
